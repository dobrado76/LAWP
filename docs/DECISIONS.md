...
| D47 | A code block with `preview.kind: "iframe"` is a **DOM page**. Studio shows **Preview / HTML / CSS** tabs (fixture source is inspectable, not hidden behind the renderer). The JS editor uses **`dom-v1`** completions (`document`, `querySelector` and the element it returns, plus selectors from that page). Fox grids stay `player-v1` | Learners can see the markup they are selecting |
