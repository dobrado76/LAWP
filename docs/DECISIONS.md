...
| D43 | Graphical coding uses an app-owned **`player-v1`** harness (`Player.move` / `rotate` / `scale` / `say`). Stubs inject into the sandbox; main applies `play-log.json` to `world-v1` and grades **that** world. Studio `view.kind: "grid"` is a skin, not a second engine. Pack strings are never `eval`’d. Bundled stubs are app code, not pack-fingerprint prose | One reusable play API; same trust gate as other spawned code |
