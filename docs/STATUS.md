...
- **Circuits** (`lawp.circuits.basics`): 23-lesson thorough-beginner electricity path (placement through series/parallel, shorts/fuses, meters, kept creation). Original SVG diagrams in explain, image/hotspot/place checks, and `world-v1` graph view. Why / misconceptions, hints, transfer, export
- **Python** and **JavaScript** code lessons: spawn in a temp sandbox, stdout / AST / assert checks, Why, hints. The Python sandbox is on `sys.path`, so a lesson can ship a companion module and a hidden test can `import main` (D59)
- **Python zero-to-hero path** (`lawp.python.foundations`): 114 lessons over six tracks — values, the fox grid, functions, collections, errors, text, files, the process, modules, objects, generators and decorators, async, and a tested capstone. Hidden `python-assert` on every code lesson
...
- Studio editor: CodeMirror, language lint (Python names the missing colon, `=` vs `==`, and a Python 2 `print` — D60), Player autocomplete on fox grids in **both** languages, read-only panes for companion modules and fixtures the exercise reads, **DOM autocomplete** (`document`, `querySelector`, members for *that* node, page selectors) on iframe lessons. The cursor’s selector outlines the match in Preview. Those lessons also have **Preview / HTML / CSS** tabs so the fixture source is inspectable. A **Console** under the editor stays empty until Run, then shows only `console.log` / `print`. Cartridge file paths stay hidden. Learner **drafts** under `learners/<id>/drafts/` are per lesson; a flush on leave cannot write one starter into the next. **Restart lesson** reloads the authored starter and will not save the old buffer on top of that
- Studio bar: Back / Next / Hint / Restart are icons with tooltips; **Run** previews code. A lesson that both teaches and has a task opens **Learn** then **Try** (D57). Learn keeps the two-column studio grid: theory left, questions + Continue right. Continue waits for answers; Submit stays on the Try work pane. Fenced explain snippets render as highlighted blocks. Every graded step uses one **Submit → Correct! / Incorrect! → Next** control. A lesson with a question and a fox/code task is graded together — Correct only when the question is right and the goals are met. Last lesson: **Return to library** plus Congratulations and a first-try score; Correct / Incorrect shows on Try. Grid SUCCESS / FAIL overlay stays.
...
| Area | Honest state |
| --- | --- |
| React pack | Lessons load; learner JS runs through **Node**, not a sandboxed iframe preview. It is the one pack still short of [COURSE_SPEC.md](COURSE_SPEC.md) — six lessons with no retrieval blocks and mostly no hidden tests — so `tests/course-spec.test.ts` exempts it by name until it is rebuilt |
| Author | Forms cover shipped block and check kinds; not a drag-canvas CMS |
| Python catalog | 114 lessons on the locked path (Foundations → Fluency → Files and the machine → Structure → Language → Craft), generated by `scripts/author-py-expert.mjs`. Tests assert path order, that every starter fails honestly while running cleanly, and that every assist solves its lesson. No NumPy, pandas, ML, or web frameworks |
...
The full contract for building one — in any subject — is [COURSE_SPEC.md](COURSE_SPEC.md) (D61), and its subject-agnostic half runs as `tests/course-spec.test.ts` against every bundled pack.
