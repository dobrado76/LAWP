...
- Runtime Library: bundled `resources/packs` + `%APPDATA%\LAWP\packs` (overlay lesson zip vs full pack zip, D37)
...
- **`player-v1` grid:** `Player.move` / `rotate` / `scale` / `say`; main applies `play-log.json` and grades that world (D43)
...
| JS / Python catalogs | Foundations and Course 1 material; not an expert-length catalog |
