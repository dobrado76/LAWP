...
- When a lesson both teaches and has an exercise, Studio is **two beats**, inferred from the blocks (D57): **Learn** (theory left, questions + **Continue to the exercise** right — same two-column grid as a check-only quiz) then **Try** (task prompt left, editor or world + Submit right). Continue waits for answers the same way Submit does. **Back to the idea** returns to Learn. Check-only and play-only stay one screen
- Learn left: Markdown explain / reflect only. A fenced snippet (` ```javascript `) is a labeled, highlighted block in the editor palette — never leftover backticks around uncolored lines. Inline `` `code` `` stays in the sentence. Learn right: the questions plus **Continue to the exercise** (no editor yet)
- Try left: task prompt + predict-after / objectives + grade result. Try right: **activity view** (`world-v1` graph/list/grid) **or** the shared `CodeEditor` (gutter, highlighting, click-to-error) **or** check UI on a check-only lesson
- React: tabs for files + Preview pane
- Top studio bar: icon Back / lesson title / **the pack title, centred** / icon **Run** (play) · icon Hint · icon Restart / icon Next (tooltips). The centred pack title answers "which course am I in" without going back to the Library; it ellipsises rather than pushing the controls around. On a paged lesson the title also reads **Learn** or **Try**. **Submit lives only in the Try work pane.** After a **correct** Submit, that same control becomes Next; an incorrect grade keeps Submit so they can change the answer and try again. On the last lesson the control is **Return to library** (never a dead Next), and the left pane shows Congratulations plus a first-try score (wrong then fixed is not 100%)
